...
def vec_GMAB_var_maturity(world_args, t_end, A, ec, rc, cr):
    t_now, t_max, dt, r, sig2, eZ, S0, S = set_world(world_args)
    
    pres_vals = np.ones(A.shape)
    pres_vals[1:] = np.divide.accumulate(1+r*dt)
    
    # get A as fxn of t
    A = A[0] * np.ones(A.shape)
    rates = np.multiply.accumulate(eZ - (ec+rc)*dt)
    A[1:] *= rates

    # from A get inflow
    inflows = np.zeros(A.shape)
    inflows[:-1] = A[:-1] * (ec+rc) * dt
    # multiply inflows by pres_val
    inflows *= pres_vals
    inflow_cum = np.cumsum(inflows)
    
    #extract payout at each year's end
    bb = (1+cr)**(dt) * np.ones(A.shape)
    bb = A[0] * np.multiply.accumulate(bb)/ (1+cr)**dt
    
    anniversaries = slice(0,int(t_end/dt)+1,int(1/dt))
    gmabs = np.maximum(bb[anniversaries] - A[anniversaries], 0)
    gmabs *= pres_vals[anniversaries]

    return inflow_cum[anniversaries] - gmabs, None, None
...
def make_future_gen(world_args, N = None):
  ...
  while N is None or count < N :
    ...
    if no_hist:
      ...
      yield set_world((t_now, t_max, dt, r, sig2, neZ, S0, None))
...
def multi_step_val_by_rep(world_args, N, contract, *args):
  future_gen = make_future_gen(world_args, N)
  samp = [contract(future_world, *args)[0] for future_world in future_gen]

  mean = list(map(np.mean, map(list, zip(*samp))))
  std = list(map(np.std, map(list, zip(*samp))))
  return mean, list(std/np.sqrt(N)), [N]*len(samp)
...
def organize_data(shock_dat):
    ...
    S_val = list(zip(it.count(),S_vals))
    r_val = list(zip(it.count(),r_vals))
    Sr_val = dict(map(lambda tup : ((tup[0][0], tup[1][0]), (tup[0][1], tup[1][1])),
                        it.product(S_val, r_val)))
    ...
    return Sr_val, Sr_index, tru_vals, err, NN
